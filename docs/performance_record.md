...
|| 2026-01-28 | WIP | **五级流水线** | 40.6M → 36.5M (**10.2%**) | 0.018 → 0.020 | 流水线版本成功运行 microbench, CPI 从 55.74 降至 50.06 |

## 五级流水线性能数据 (2026-01-28)
...
| 数据冒险处理 | 完整转发 (EX→ID, MEM→ID, WB→ID) |
| Load-use 冒险 | 阻塞 1 周期 |
| 控制冒险处理 | 静态预测不跳转 + flush |
| JAL/JALR | EX 阶段检测，flush IF/ID |
| 异常处理 | 精确异常（WB 阶段处理） |

### 多周期 vs 流水线对比 (microbench test)

| 指标 | 多周期版本 | 流水线版本 | 变化 |
|------|------------|------------|------|
| 总周期数 | 40,625,543 | 36,489,609 | **-10.2%** |
| 指令数 | ~729K | ~729K | - |
| CPI | 55.74 | 50.06 | **-10.2%** |
| 实际运行时间 | 21.0 ms | 21.0 ms | - |
| Scored time | 10.0 ms | 10.0 ms | - |

### 关键修复：JALR 返回地址 Bug

**问题**：JALR 指令将跳转目标 `(rs1+imm)` 写入 rd，而非返回地址 `(PC+4)`

**影响**：函数调用后返回地址错误，导致函数被重复执行
...
// 修复后：
7'b1100111: alu_result = in_pc + 32'd4;  // 正确：返回地址
wire [31:0] jalr_target = (in_rs1_data + in_imm) & 32'hFFFFFFFE;  // 跳转目标
```
...
| 测试 | 状态 |
|------|------|
| microbench (qsort, fib, sieve 等) | ✅ PASS |
| cpu-tests (dummy, add, bubble-sort, fact, fib, load-store) | ✅ PASS |
