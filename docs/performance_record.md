...
|| 2026-01-28 | 27b8cbb | **五级流水线** | 30.6M → 27.5M (**10.2%**) | 0.018 → 0.020 | CPI 从 55.66 降至 49.94, 修复 JALR 返回地址 bug |

## 五级流水线优化效果 (2026-01-28)
...
|| 总周期数 | 30,593,144 | 27,474,092 | **-10.2%** |
|| 退役指令数 | 549,606 | 550,137 | +0.1% |
|| microbench 结果 | PASS | PASS | - |
...
|| 数据转发 | EX→ID, MEM→ID, WB→ID |
|| Load-Use 冒险 | 插入 1 cycle stall |
|| 控制冒险 | 分支在 EX 解析，flush IF/ID |
|| fence.i | 刷新流水线和 I-Cache |

### 关键 Bug 修复

1. **JALR 返回地址错误**: JALR 应将 PC+4 写入 rd，而不是跳转目标地址
2. **LSU 数据锁存**: load_result 应使用锁存的 mem_result
3. **MEM 转发条件**: 需检查 ex_mem_valid 确保转发数据有效
