...
| 阶段5 | 实现控制冒险处理 + 分支/跳转测试 | 🔄 进行中（基本框架完成） |
| 阶段6 | 实现异常和 fence.i + 完整测试 | ⏳ 待开始 |
| **D-Cache 集成** | 集成 DCache 到流水线减少访存延迟 | ⏳ **关键优化** |
...
**当前性能瓶颈 (2026-01-28)**:
- 由于 IFU/LSU 仲裁，CPI 约为 200（远高于多周期版本的 56）
- 数据初始化循环（Flash→PSRAM 拷贝）频繁的 LSU 读操作阻塞 IFU
- 需要 D-Cache 来减少 LSU 对慢速内存的访问频率
