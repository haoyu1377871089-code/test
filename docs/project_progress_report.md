...
> 更新日期：2026-01-28
> 最新更新：2026-01-28 - 完成阶段2-6的主要修复和实现，添加测试脚本框架
...
**已修复问题 #6：LSU out_valid 持续问题**

- **原因**：当不需要访存的指令（如 branch）通过 LSU 时，`out_valid=1` 在 WBU 忙时不会清零，导致重复提交
- **修复**：修改 `LSU_pipeline.v` 的 `S_IDLE` 状态逻辑，先处理输出被消费的情况，再处理新输入，确保每条指令只输出一个周期的 valid

**已修复问题 #7：MEM/WB 寄存器重复更新**

- **原因**：`mem_wb_valid` 在 `lsu_out_valid` 为 1 时被设置为 1，但如果 `lsu_out_valid` 保持多个周期，`mem_wb_valid` 也会保持多个周期，导致重复提交
- **修复**：使用握手协议，`mem_wb_valid` 只在数据被 WBU 消费后清零，新数据只在当前没有有效数据或数据刚被消费时更新

**已修复问题 #8：id_ex_valid flush 逻辑错误**

- **原因**：当 `flush_id` 为 1 时，`id_ex_valid` 被清零，导致已经在 EX 阶段的指令也被冲刷
- **修复**：修改 `id_ex_valid` 的更新逻辑，指令从 EX 阶段移出时清零，新指令进入时设置，flush 时只阻止新指令进入，不影响已在 EX 阶段的指令

**已修复问题 #9：RAW 冒险检测使用错误信号**

- **原因**：RAW 检测使用 `mem_wb_valid`，但 WBU 接收数据后需额外周期才写入寄存器堆
- **修复**：RAW 检测改用 WBU 的写入信号（`wbu_rf_wen`, `wbu_rf_waddr`），同时检测当前周期写入和待写入的数据

### 4.4 新发现问题 (2026-01-28 深入调试)
