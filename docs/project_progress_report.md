...
| 阶段2 | 实现流水线连接 + 简单测试（无 RAW 依赖的程序） | 🔄 进行中（dummy 未通过） |
| 阶段3 | 实现 RAW 阻塞 + 带依赖的测试用例 | 🔄 进行中（已实现基础逻辑） |
| 阶段4 | 实现转发技术 + 验证性能提升 | ⏳ 待开始 |
| 阶段5 | 实现控制冒险处理 + 分支/跳转测试 | 🔄 进行中（flush 问题待修复） |
...
1. **确认 flush 作用范围**
   - 仅冲刷 IF/ID（以及必要时的 ID/EX），避免误杀 MEM 阶段的 store/load
   - 观察点：`flush`、`if_id_valid`、`id_ex_valid`、`ex_mem_valid`

2. **核对 MEM/WB 握手机制**
   - `mem_wb_valid` 只在 WBU 接收（`mem_wb_ready && mem_wb_valid`）时清零
   - 观察点：`lsu_out_valid`、`lsu_out_ready`、`mem_wb_valid`、`mem_wb_ready`

3. **还原指令提交顺序**
   - 记录 `pc/inst` 提交序列，确认 `lw → addi → sw → lw → bge` 是否被完整提交
   - 对比分支指令是否重复提交

4. **短期验证指标**
   - `dummy` 测试运行进度（是否走出 `uart_init` 延迟循环）
   - `perf_flush_branch`/`perf_stall_raw` 计数是否异常偏高
